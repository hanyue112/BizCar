...
V2: For some reason, BizCover.Api.Cars cannot inject the HTTPCLIENT, more investigation may required. - fixed by copy and paste the class file in visual studio (possible Micorsoft BUG)

For some reason, DLL provided in the test cannot working, and mocked version included as a mutiple-threading solution

V2: Fixed a BUG, when trying to update a non-existing car, the car will be created using the Id given in the request rather than the Id form the IdSquence, this may cause data lost.